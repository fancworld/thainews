...
สไตล์: เตือนภัย/ให้ความรู้ น้ำเสียงห่วงใยแบบพิธีกรข่าว ไม่มีตัวคนพูดปรากฏในคลิป
(คลิปจะประกอบด้วยรูปที่สร้างขึ้น + เสียงพากย์เท่านั้น)

กติกาการเขียน:
1. แบ่งสคริปต์เป็น scene ละ 1 ประโยคสั้นๆ (พูดได้ใน 5-8 วินาที/scene) รวม 5-7 scene ต่อคลิป (~30-45 วินาที)
2. Scene แรกต้องเป็น Hook ที่ดึงความสนใจผู้ชมภายใน 3 วินาทีแรก
3. Scene สุดท้ายต้องเป็นคำเตือน/คำแนะนำเชิงป้องกัน (CTA) ให้ผู้ชมนำไปใช้ได้จริง
4. ห้ามระบุชื่อเฉพาะของผู้เกี่ยวข้องในข่าว และห้ามอ้างอิงถึงภาพจริงของบุคคลในข่าว
   ให้เล่าเชิงสรุปเหตุการณ์เพื่อการศึกษา/เตือนภัยสาธารณะเท่านั้น
...
6. image_prompt เป็นภาษาอังกฤษ (คำสั่งให้โมเดลสร้างภาพเข้าใจ) บรรยายภาพประกอบที่ "ดัดแปลงจาก
   ...
   แบบตัวหนา สไตล์โปสเตอร์เตือนภัย (สีแดง/เหลือง contrast สูง อ่านง่าย) สะกดตามที่ให้เป๊ะๆ ห้ามเพี้ยน
   เช่น: "...overlay the {lang_label} text '<overlay_text>' (written in {lang_label} script/characters)
   as bold red/yellow high-contrast poster-style typography, spelled EXACTLY as given, clearly
   legible, positioned at top or bottom third..."
7. ทุก image_prompt ต้องคุมโทนภาพ/สไตล์การถ่าย/แสง ให้ตรงกับ STYLE_GUIDE นี้เหมือนกันทุก scene
   ...
   แบรนด์คลิปดูสอดคล้องกันข้ามภาษา):
...
ยกเว้น image_prompt ที่เป็นภาษาอังกฤษ):
{{
  "headline": "พาดหัวคลิปสั้น กระชับ ดึงดูด ไม่เกิน 12 คำ",
  "scenes": [
    {{"narration": "บทพากย์ของฉากนี้ภาษา {lang_label}", "overlay_text": "ข้อความสั้นๆ ภาษา {lang_label} บนภาพ", "image_prompt": "English description for image generation, must include exact instruction to render the {lang_label} overlay_text as bold poster typography"}}
  ]
}}
"""
